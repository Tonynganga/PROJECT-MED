...
class Appointment_setting_ps_API(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        obj = get_object_or_404(
            self.queryset, **{self.lookup_field: request.user.id})
...
class Available_time_choice_ps_API(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        self.queryset = Available_time_choices_per_station.objects.filter(
            aps_per_station=self.kwargs['aps_per_station'])
        return super().list(request, *args, **kwargs)

    @action(detail=False, methods=['get'])
    def list_for_date(self, request, *args, **kwargs):
        filled_time_queryset = Filled_date_time_choices_per_station.objects.filter(
            aps_per_station=self.kwargs['aps_per_station'], booked_date=self.kwargs['date'])
        queryset = Available_time_choices_per_station.objects.filter(
            aps_per_station=self.kwargs['aps_per_station'])
        for item in filled_time_queryset:
            queryset = queryset.exclude(
                available_appointment_time=item.booked_time)
    ...
    def create_from_list(self, request):
        ...
        for time in available_appointment_time_list:
            data = {"aps_per_station": aps_per_station,
                    "available_appointment_time": time}
...
class Booked_appointments_API(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Booked_appointments.objects.all().filter(
            doctor_account=self.request.user)
        return queryset

    def create(self, request, *args, **kwargs):
        aps_query = Appointment_settings_per_station.objects.get(
            id=request.data['appointment_id'])
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save(patient_account=request.user,
                        doctor_account=aps_query.doctor_account)
        headers = self.get_success_headers(serializer.data)
        return Response(serializer.data, status=status.HTTP_201_CREATED, headers=headers)
...
class Appointment_setting_and_requirements_API(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        for time in available_appointment_time_list:
            data = {
                "aps_per_station": appointment_setting_serializer.data["id"], "available_appointment_time": time}
